...
class KDNet(nn.Module):
    def __init__(self, tag='teacher', layers=50, bins=(1, 2, 3, 6), dropout=0.1, classes=2, zoom_factor=8, temperature = 1, alpha = 0., use_ppm=True, criterion=nn.CrossEntropyLoss(ignore_index=40), pretrained=True):
        ...
        self.alpha = alpha
        self.student_net = PSPNet(tag='student', layers=18, bins=(1, 2, 3, 6), dropout=0.1, classes=2, zoom_factor=8, use_ppm=True, criterion=nn.CrossEntropyLoss(ignore_index=40), pretrained=True)
        self.teacher_loader = teacher_loader()

    def distillation(self, student, aux, teacher, truth, temperature, alpha):
        criterion = nn.CrossEntropyLoss(ignore_index=40)
        ...
        ce_loss = F.cross_entropy(student, truth,ignore_index=40)
