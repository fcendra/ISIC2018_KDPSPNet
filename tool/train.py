...
def get_parser():
    parser = argparse.ArgumentParser(description='PyTorch Semantic Segmentation using ISIC2018 Dataset')
    parser.add_argument('--config', type=str, default='config/isic2018/isic2018_pspnet50.yaml', help='config file')
    parser.add_argument('opts', help='see config/isic2018/isic2018_pspnet50.yaml for all options', default=None, nargs=argparse.REMAINDER)
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if args.arch == 'psp':
        ...
        modules_new = [model.student_net.ppm, model.student_net.cls, model.student_net.aux]
...
def train(train_loader, model, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        output, main_loss, aux_loss, kl_loss, ce_loss = model(input, target)
        ...
        n = input.size(0)
        if args.multiprocessing_distributed:
            main_loss, aux_loss, loss, kl_loss, ce_loss = main_loss.detach() * n, aux_loss * n, loss * n, kl_loss * n, ce_loss * n  # not considering ignore pixels
            count = target.new_tensor([n], dtype=torch.long)
            dist.all_reduce(main_loss), dist.all_reduce(aux_loss), dist.all_reduce(loss), dist.all_reduce(kl_loss), dist.all_reduce(ce_loss), dist.all_reduce(count)
            n = count.item()
            main_loss, aux_loss, loss, kl_loss, ce_loss = main_loss / n, aux_loss / n, loss / n, kl_loss / n, ce_loss / n,
        ...
        if (i + 1) % args.print_freq == 0 and main_process():
            logger.info('Epoch: [{}/{}][{}/{}] '
                        ...
                        'CE_Loss {ce_loss_meter.val:.4f} '.format(epoch+1, args.epochs, i + 1, len(train_loader),
